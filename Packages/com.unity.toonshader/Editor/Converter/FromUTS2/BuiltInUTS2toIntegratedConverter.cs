    ...
    private static string GetPackageFullPath() {
        ...
        void RestoreShaderGUID(UTS3GUI.RenderPipeline renderPipeline)
        {
            ...
            if (renderPipeline == UTS3GUI.RenderPipeline.Legacy)
            {
                ...
                using (FileStream fs = new FileStream(filePath, FileMode.Open)) { using (TextWriter tw =
 new StreamWriter(fs, Encoding.UTF8, 1024, true)) { tw.Write(content); } fs.SetLength(fs.Position); }
            }
    ...
    void ConvertBuiltInUTS2Materials(string[] guids) {
        foreach (var guid in m_ConvertingMaterialGuids) {
            ...
            UTS3GUI.UTS_TransparentMode transparencyEnabled = (UTS2Info.m_renderQueue == UTS2RenderQueue.Transparent)
                ? UTS3GUI.UTS_TransparentMode.On
                : UTS3GUI.UTS_TransparentMode.Off;
    ...
    void BasicLookdevs(Material material) {
        if (material.HasProperty(UTS3GUI.ShaderPropUtsTechniqe)) //ThreeColorToon or ShadingGradeMap
        {
            if (UTS3GUI.MaterialGetInt(material, UTS3GUI.ShaderPropUtsTechniqe) == (int)UTS3GUI.UTS_Mode.ThreeColorToon) //DWF
            {
                ...
                material.SetFloat(ToonConstants.SHADER_PROP_TOON3D_1ST_SHADE_COLOR_FEATHER,
                    material.GetFloat(ToonConstants.SHADER_PROP_TOON3D_BASE_SHADE_FEATHER));
                ...
                material.SetFloat(ToonConstants.SHADER_PROP_TOON3D_BASE_SHADE_FEATHER,
                    material.GetFloat(ToonConstants.SHADER_PROP_TOON3D_1ST_SHADE_COLOR_FEATHER));
    ...
    public override InstalledStatus CheckSourceShaderInstalled() {
